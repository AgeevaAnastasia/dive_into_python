    ...
    def validate(self, value):
        if isinstance(value, str):
            if not value.replace(' ', '').isalpha():
                raise ValueError(f'Значение {value} должно быть состоять только из букв')
            if not value.istitle():
                raise ValueError(f'Каждое слово в значении {value} должно начинаться с заглавной '
                                 f'буквы, а остальные буквы должны быть строчные')
        elif isinstance(value, int):
            if self.min_value is not None and value < self.min_value:
                raise ValueError(f'Значение {value} должно быть больше или равно {self.min_value}')
            if self.max_value is not None and value >= self.max_value:
                raise ValueError(f'Значение {value} не должно быть больше {self.max_value - 1}')
        else:
